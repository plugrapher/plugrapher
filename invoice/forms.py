...
class ClientForm(forms.ModelForm):
    class Meta:
        model = Client
        fields = ['clientName', 'clientLogo', 'addressLine1', 'county', 'postalCode', 'phoneNumber', 'emailAddress', 'taxNumber']
...
class ProductForm(forms.ModelForm):
    class Meta:
        model = Product
        fields = ['title', 'description', 'quantity' ,'currency','rates']
...
class ProductSearchForm(forms.Form):
    search_query = forms.CharField(
        label="Search",
        max_length=100,
        widget=forms.TextInput(attrs={'class': 'form-control form-control-dark w-100'}),
    )
