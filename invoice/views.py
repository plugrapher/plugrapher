...
def viewPDFInvoice(request, slug):
    ...
    #Get Client Settings
    p_settings = Settings.objects.get(clientName='Zao Kuu')
    ...
    if len(products) > 0:
        for x in products:
            y = float(x.quantity) * float(x.rates)
            invoiceTotal += y
...
def companySettings(request):
    company = Settings.objects.get(clientName='Zao Kuu')
...
def search_view(request):
    ...
    if form.is_valid():
        ...
        if search_query:
            # Filter the products based on the search query (customize this filter according to your needs)
...
#def client_feedback(request):
 #   patient_fed = Client.objects.get(admin=request.user.id)
  #  feedback = ClientFeedback.objects.filter(patient_id=patient_fed)
   # context = {
    #    "feedback":feedback
    #}
...
#def client_feedback_save(request):#
  #  if request.method == "POST":
   ...
   #     staff_obj = Client.objects.get(admin=request.user.id)

     
    #    add_feedback =ClientFeedback(Client_id=staff_obj, feedback=feedback, feedback_reply="")
   #     add_feedback.save()
    #    messages.success(request, "Feedback Sent.")
    #    return redirect('client_feedback')

#def Clientdeletefeedback(request,pk):
   # try:
      #  fed=ClientFeedback.objects.get(id=pk)
      #  if request.method == 'POST':
      #      fed.delete()
  ...
  #      return redirect('patient_feedback')
   # return render(request,'users/sure_delete.html')
